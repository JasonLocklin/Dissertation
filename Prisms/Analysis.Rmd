...
p <- p +  geom_line() + scale_x_continuous(breaks=c(5,15,30,60)) +
  scale_y_continuous(breaks=c(5,15,30,60)) + ylab("Estimate") +
  stat_smooth(aes(group = 1), method = "glm", formula = y ~ log(x)) + 
  facet_grid(. ~ Prisms) + ggtitle("Temporal Estimation Across Intervals")
p
pl <- p # for the figure later.
```
...
Running on log transform:
```{r TE_log, fig.width=12, cache=FALSE}
TE2 <- ddply(TE[as.numeric(TE$Session) < 3 & TE$Subject != "27",],
             .(Subject, Session, Prisms, TrialDuration), 
             summarize, Response = round( median(Response, na.rm=TRUE), 0) )
TE2$TrialDuration <- as.numeric(as.character(TE2$TrialDuration)) #switch to numeric
...
p <- ggplot(data = TE2, aes(x = log(TrialDuration), 
                           y = log(Response), group = Subject))
...
Alternatively, log transform the data and compute lm() for each participant
to get a slope measure. 

```{r TE_Slope}
TE2 <- ddply(TE, .(Subject, Session, Prisms), summarize, 
      r_squared = summary(lm(log(Response) ~ log(as.numeric(as.character(TrialDuration)))
                 ))$r.squared
      )
summary(TE2)

#Worst case is 10 pre prisms
t <- TE[TE$Subject == '10' & TE$Session == '1',]
summary(lm(log(t$Response) ~ log(as.numeric(as.character(t$TrialDuration)))))

TE2 <- ddply(TE, .(Subject, Session, Prisms), summarize, 
      slope = summary(lm(log(Response) ~ log(as.numeric(as.character(TrialDuration)))
                 ))$coefficients[2,1] )
TE2
TE_tbl <- TE2


p <- qplot(data = TE2, x=Session, y=slope, geom="bar", stat="identity", 
       fill=Prisms, width = 0.8) + facet_wrap(~ Subject) + 
       scale_fill_grey(start=0.2, end =0.5) +
      ggtitle("Individual Temporal Estimation Accuracy")
```

Now, create the figure:
```{r}
require(grid)
pdf("fig_TE.pdf", paper='USr', width=11)
pushViewport(viewport(layout = grid.layout(1, 2)))
print(p + theme(plot.title = element_text(size = rel(1.5)), 
          axis.text = element_text(size = rel(1.5)),
          axis.title = element_text(size = rel(1.5))), vp = viewport(layout.pos.row = 1, layout.pos.col = 1))
print(pl + theme(plot.title = element_text(size = rel(1.5)), 
          axis.text = element_text(size = rel(1.5)),
          axis.title = element_text(size = rel(1.5))), vp = viewport(layout.pos.row = 1, layout.pos.col = 2))
dev.off()
...
p <- qplot(data = SWM2, x=Prisms, y=H_minus_F, geom="bar", stat="identity", 
        width = 0.8, ylim=c(0,100), ylab = "Hits minus false alarms", 
        xlab = "Dark gray bar = RBD controls and 
light gray bar = healthy controls (Ferber, 2006).") + 
        facet_wrap(~ Subject) +  
        geom_abline(aes(intercept=79, slope=0), size=1, alpha=0.9) +
        geom_abline(aes(intercept=79, slope=0), size=2*8.7, alpha=0.1) +
        geom_abline(aes(intercept=89, slope=0),  size=1, alpha=0.9) +
        geom_abline(aes(intercept=89, slope=0),  size=2*6.1, alpha=0.7) +
        ggtitle("Spatial Working Memory Test") 

p

pdf("fig_SWM.pdf", paper='USr', width=11)
p + theme(plot.title = element_text(size = rel(1.5)), 
          axis.text = element_text(size = rel(1.5)),
...
Everything but Sj 10, session 2 looks fine. Looking at those trials:
```{r}
LB[LB$Subject=="10" & LB$Session=="2",]
```
...
sign <- data.frame(rep(1.5,12), rep(2,12), c("","","*","*","*","*","","","*","*","",""))
LB2$x <- rep(1.55,12)
LB2$y <- rep(2,12)
LB2$sign <- c("","","*","*","*","*","","","*","*","","")
p <- qplot(data = LB2, x=as.numeric(Prisms), y=PercentError, geom="bar", stat="identity", 
    ...
    geom_text(aes(x, y, label=sign, group=NULL), alpha=0.7, size=10)

p

pdf("fig_LB_Prisms.pdf", paper='USr', width=11)
p + theme(plot.title = element_text(size = rel(1.5)), 
          axis.text = element_text(size = rel(1.5)),
          axis.title = element_text(size = rel(1.5)),
...
NT2$Drawing <- factor(NT2$Drawing)
levels(NT2$Drawing) <- c('-','+')
levels(NT2$Prisms) <- c('Pre','Post')

require(reshape2)
clin <- dcast(melt(NT2, id.vars=c('Patient', 'Prisms')), 
              Patient ~ Prisms + variable)

names(clin)  <- c("Subject", "Star(pre)","Bell(pre)","Copy(pre)", 
                  "Star(post)", "Bell(post)", "Copy(post)")
clin <- clin[c(1,2,5,3,6,4,7)]

  # Line Bisection
names(LB)[4] <- 'value'
LB_m <- dcast(LB, Subject ~ Prisms, fun=mean )
LB_m$pre <- round(LB_m$pre,1)
LB_m$post <- round(LB_m$post,1)
names(LB_m) <- c('Subject', 'LB(pre)','LB(post)')
                  
  # TE
names(TE_tbl)[4] <- 'value'
TE_tbl <- dcast(TE_tbl, Subject ~ Prisms, mean)
TE_tbl$pre <- round(TE_tbl$pre,1)
TE_tbl$post <- round(TE_tbl$post,1)
names(TE_tbl) <- c('Subject','TE(pre)','TE(post)')

  # SWM
      
SWM_tbl <- SWM2[c(1,2,5)]
names(SWM_tbl)[3] <- 'value'
SWM_tbl$value <- round(SWM_tbl$value) 
SWM_tbl <- dcast(SWM_tbl, Subject ~ Prisms, mean)
names(SWM_tbl) <- c('Subject','SWM(pre)','SWM(post)')

#Now combine
tbl  <- merge(dem, clin, by='Subject')
tbl2 <- merge(LB_m, TE_tbl, by='Subject')
tbl2 <- merge(tbl2, SWM_tbl, by='Subject', all.x=TRUE)
tbl2 <- tbl2[order(tbl2$Subject),]
tbl <- tbl[order(tbl$Subject),]
rownames(tbl)<- seq(1,6)
rownames(tbl2)<- seq(1,6)
tbl <- tbl[order(tbl2[['LB(pre)']]),]
tbl2 <- tbl2[order(tbl2[['LB(pre)']]),]
rownames(tbl) <-tbl$Subject
rownames(tbl2) <- tbl2$Subject
tbl <- tbl[-1]
tbl2 <- tbl2[-1]

require(gridExtra)
pdf("tbl_Prisms.pdf", paper='USr', width=11)
grid.arrange(tableGrob(tbl), tableGrob(tbl2), nrow=2, sub="tbl_Prisms(a and b)", clip=FALSE)
dev.off()

require(xtable)
print.xtable(xtable(tbl), type="latex", file="tbl_Prisms_a-raw.tex")
print.xtable(xtable(tbl2), type="latex", file="tbl_Prisms_b-raw.tex")
