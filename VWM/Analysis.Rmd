...
t.test(q[2], alternative="greater")
```
Huh, significant. Note that this would not reach significance if
...
d2 <- d
shapiro.test(d2$CES)
d2$CES <- d2$CES + 10 -  min(d2$CES) #Shift CES to allow for log transform
shapiro.test(log(d2$CES))
...
fit6c <- glm(Group~ log(CES) + VWM_NT + VWM_SG,data=d2,family=binomial())
plot(fit6b)
anova(fit6b, test="Chisq")
library(xtable)
print.xtable(xtable(anova(fit6b, test="Chisq")), type="latex", 
             file="aod-raw.tex")

plot(fit6c)
anova(fit6c, test="Chisq")
drop1(fit6c, test="Chisq")
# VWM_NT doesn't contribute significantly and can be dropped
fit6d <- glm(Group~ VWM_SG + log(CES),data=d2,family=binomial())
anova(fit6d, test="Chisq")

#RE-run without high leveredge points to check:
fit6b <- glm(Group~ log(CES) * VWM_NT * VWM_SG,
             data=d2[c(-1,-6,-14,-16),],family=binomial())
plot(fit6b)
anova(fit6b, test="Chisq")
