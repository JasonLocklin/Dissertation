...
COR <- cbind(COR, TESTS_P, by="Subject", label_size = 5)[,c(1:5,7,8,9)]
ezCor(COR[,c(2:6,8)], label_size = 5)
```
...
```{r}
ezCor(COR[!is.na(COR$aa),][,c(2:6,8)], label_size = 5)
```
...
origional plot.)
```{r}
ezCor(COR[,c(4,5,6,8)], label_size = 5)
```
...
### Comparison with simple leftward RT

```{r}
lrt_tbl <- merge(COR, LRT, all.x=FALSE)[,c(1,4,5,6,8,9)]
rownames(lrt_tbl) <- lrt_tbl$Subject
lrt_tbl <- lrt_tbl[-1]
lrt_tbl<- lrt_tbl[order(lrt_tbl$Bisection),]
ezCor(lrt_tbl, label_size = 5)
```


```{r}
grid.table(round(lrt_tbl,2))
```
