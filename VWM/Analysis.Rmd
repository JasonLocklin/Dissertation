...
COR <- cbind(COR, TESTS_P, by="Subject", label_size = 5)[,c(1:5,7,8)]
ezCor(COR[,c(2:7)], label_size = 5)
```
...
```{r}
ezCor(COR[!is.na(COR$aa),][,c(2:7)], label_size = 5)
```
...
origional plot.)
```{r}
ezCor(COR[,c(4,5,6,7)], label_size = 5)
```
