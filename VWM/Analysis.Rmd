...
pdf("fig.VWM_Precision.pdf", paper='USr', width=11)
p
dev.off()
```
...
pdf("fig.VWM_1Target.pdf", paper='USr', width=11)
p + theme_gray()
dev.off()
```
...
```{r fig.VWM_MTarget}
require(grid)
pdf("fig.VWM_MTarget.pdf", paper='USr', width=11)
...
c
pdf("fig.COVAT.pdf", paper='USr', width=11)
c
dev.off()
```
...
## Demographics and Clinical table (tbl.VWM)
...
pdf("tbl.VWM.pdf", paper='USr', width=11)
grid.arrange(tableGrob(tbl), sub="tbl.VWM", clip=FALSE)
dev.off()
```


Now, plot the distributions to be used in fig.emrich2012. 
...
svg(filename="fig.Emrich2012.svg", width=4, height=6)
multiplot(m,n,o)
dev.off()
