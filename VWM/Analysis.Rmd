...
The data file I received is a complicated excel file with lots of information. I
...
### Are the patients able to encode and recall colour? (Precision) 
...
p + geom_bar(stat = "identity", width=0.5) +
    ...
    xlab("") +
    ylab("Precision (SE)") + 
    ggtitle("Response Precision for Single Square Condition") +
    scale_y_continuous(breaks=0:20*4) +
    theme_gray() +
    geom_jitter(data=simple, aes(x = Subj_Group, y = value), position = position_jitter(width = .1))
```
...
ANOVA
```{r}
fit <- aov(log(value) ~ Subj_Group, data=simple) 
summary(fit)
plot(fit)
shapiro.test(log(simple$value))
TukeyHSD(fit)
```

With 3 problematic data points removed
```{r}
shapiro.test(simple$value[c(-6, -24, -14)])
fit <- aov(value ~ Subj_Group, data=simple[c(-6, -24, -14),]) 
summary(fit)
plot(fit)
```
...
#### Multi-square condition

Running the same analysis for the multi-square condition.
...
p + geom_bar(stat = "identity", width=0.5) +
    ...
    xlab("") +
    ylab("Precision (SE)") + 
    ggtitle("Response Precision for Multiple Square Condition") +
    scale_y_continuous(breaks=0:20*4) +
    theme_gray() +
    geom_jitter(data=simple, aes(x = Subj_Group, y = value), position = position_jitter(width = .1))
```
...
ANOVA (two squares)
```{r}
fit <- aov(log(value) ~ Subj_Group, data=simple[simple$Number_of_Items == "two",]) 
summary(fit)
plot(fit)
shapiro.test(log(simple$value))
TukeyHSD(fit)
...
ANOVA (three squares)
```{r}
fit <- aov(log(value) ~ Subj_Group, data=simple[simple$Number_of_Items == "three",]) 
summary(fit)
plot(fit)
shapiro.test(log(simple$value))
TukeyHSD(fit)
...
ANOVA (three squares)
```{r}
fit <- aov(log(value) ~ Subj_Group, 
           data=ddply(simple, c("Subj_Group", "Subj_ID", "variable"), 
...
#### ANOVA (mixed)
Sinse the log transformed data appears normal, a hierarchical model can be done.
```{r}
library(ez)
simple <- d[d$variable == "precision",]
simple$value <- log(simple$value)
a <- ezANOVA(data=simple, dv=value, wid=Subj_ID, 
        within=Number_of_Items, between=Subj_Group, return_aov=TRUE)
a
summary(a$aov)
...
# Equivelant
#a2 <- aov(data = simple, 
#          value ~ Subj_Group * Number_of_Items + 
#            Error(Subj_ID/Number_of_Items))

simple2 <- dcast(simple, Subj_Group + Subj_ID ~ ., fun=mean)
TukeyHSD(aov(data= simple2, . ~Subj_Group))
```
...
cdata <- ddply(simple, c("Subj_Group", "Number_of_Items", "variable"), 
               summarise,
               N    = length(value),
               Mean = mean(value),
               sd   = sd(value),
               se   = sd / sqrt(N) )
names(cdata)[2] <- "Items"
cdata$Items <- ordered(cdata$Items, levels=c("one","two","three"))

p <- ggplot(cdata, aes(x = Subj_Group, y = Mean, fill=Items)) +
    geom_bar(stat = "identity", width=0.5, position=position_dodge(.6)) +
    geom_errorbar(aes(ymin=Mean-se, ymax=Mean+se), 
                  width=.2, position=position_dodge(0.6)) + 
    xlab("") +
    ylab("Precision (1/SD)") + 
    ggtitle("Response Precision") +
    theme_gray() +
...
pdf("fig_VWM_Precision.pdf", paper='USr', width=11)
p
dev.off()
```

#### Looking at probability of target selection 
...
pdf("fig_VWM_1Target.pdf", paper='USr', width=11)
p + theme_gray() +
    theme(plot.title = element_text(size = rel(1.5)), 
          axis.text = element_text(size = rel(1.5)),
          axis.title = element_text(size = rel(1.5)))
...
p <- p + geom_bar(stat = "identity", width=0.5,position=position_dodge(.6)) +
    ...
    ggtitle("Mis-Binding for Multi-Target Conditions") 
...
```{r fig.height=3}
spl <- d[d$variable == "pNonTarget" & d$Number_of_Items !="one",]
cdata <- ddply(spl, c("Subj_Group", "Number_of_Items","variable"), summarise,
               N    = length(value),
               Mean = mean(value),
               sd   = sd(value),
               se   = sd / sqrt(N) )
names(cdata)[2] <- "Items"
mdata <- ddply(spl, c("Subj_Group", "variable"), summarise,
               N    = length(value),
               Mean = mean(value),
               sd   = sd(value),
               se   = sd / sqrt(N) )
...
p <- ggplot() 
p <- p + 
    geom_bar(data=mdata, aes(x = Subj_Group, y = Mean), stat = "identity", 
                  width=0.7,position=position_dodge(0)) +
    geom_errorbar(data=mdata, 
                  aes(x = Subj_Group, ymin=Mean-se, ymax=Mean+se), width=.1, 
                  position=position_dodge(.6)) + 
    geom_bar(data = cdata, aes(x = Subj_Group, y = Mean, fill = Items), 
                  stat = "identity", width=.1, position=position_dodge(0.8), 
             alpha=0.3) + 
    xlab("") +
    ylab("") + 
    ggtitle("Non-Target Selection") 
...
mdata <- ddply(simple, c("Subj_Group", "variable"), summarise,
               N    = length(value),
               Mean = mean(value),
               sd   = sd(value),
               se   = sd / sqrt(N) )
...
p <- p + 
    ...
    xlab("") +
    ylab("Probability (SE)") + 
    ggtitle("Guessing (Multiple Square Cond.)") 
...
```{r fig_VWM_MTarget}
require(grid)
pdf("fig_VWM_MTarget.pdf", paper='USr', width=11)
pushViewport(viewport(layout = grid.layout(1, 2)))
print(pg + theme_gray() + theme(legend.position = 'none')+
      theme(plot.title = element_text(size = rel(1.5)), 
          axis.text = element_text(size = rel(1.5)),
          axis.title = element_text(size = rel(1.5))), 
      vp = viewport(layout.pos.row = 1, layout.pos.col = 1))
print(pn + theme_gray() + theme(legend.position = c(0.9,0.9)) +
    theme(plot.title = element_text(size = rel(1.5)), 
          axis.text = element_text(size = rel(1.5)),
          axis.title = element_text(size = rel(1.5))), 
...
```{r}
require(ez)
shapiro.test(CES$value)



CES$SOA <- factor(CES$SOA)
CES_short <- CES[!is.na(CES$value) & ( CES$VF == "CES.L" ),]
CES_short$Subject <- factor(CES_short$Subject)
#CES_short$value <- CES_short$value + 100 -  min(CES_short$value) #Shift CES to allow for log transform
#CES_short$value <- log(CES_short$value)
shapiro.test(CES_short$value)
hist(CES_short$value)
plot(CES_short$value, pch=as.numeric(CES_short$Group))

ezANOVA(CES_short, wid = .(Subject), dv = .(value), 
        within = .(SOA), 
        between = .(Group))

#fit <- aov(data=CES_short, value ~ SOA * Group + Error(Subject/SOA))
...
Trying 5 patients would give a terrible estimate of population variance,
trying a non-parametric test:

```{r}
CES_short <- CES[!is.na(CES$value) & ( CES$VF == "CES.L" ),]
CES_short$Subject <- factor(CES_short$Subject)
kruskal.test(. ~ Group, data =dcast(CES_short, 
                                        Subject + Group ~., fun=mean))  
```
...
Post hoc tests. Single-tailed tests are performed because it is assumed neglect 
patients will be more laterally biased (re: Striemer2007).
...
Healthy Group:
```{r}
qh <- dcast(CES_short, Subject ~ ., subset=.(Group=="Older Control"),   mean)

t.test(qh[2], alternative="greater")
```

Now, for describing the individuals, calculate z-scores and plot the individual
patients over the expected distribution of healthy performance.
```{r fig_COVAT}
m = mean(qh[["."]])
sd = sd(qh[["."]])
names(q) <- .(Subject, CES)
c <- ggplot(q, aes(factor(Subject), y=CES))

# By default, uses stat="bin", which gives the count in each category
c <- c + geom_hline(aes(yintercept=m)) + 
  geom_hline(aes(yintercept=m), size=sd, alpha=0.5) +
  geom_hline(aes(yintercept=m), size=2*sd, alpha=0.25) +
  geom_bar(stat = "identity", width=0.5) +
  ggtitle("Patient Leftward Cue Effect Size (CES)") + ylab("Seconds") + xlab("Patients") +
    theme(plot.title = element_text(size = rel(1.5)), 
          axis.text = element_text(size = rel(1.5)),
          axis.title = element_text(size = rel(1.5)))
...
c
pdf("fig_COVAT.pdf", paper='USr', width=11)
c
dev.off()
```

The black line is the mean, the dark band is one standard deviation, the lighter
band is 2 standard deviations. It seems like 2 of the patients are well outside 
of normal performance while the other three are solidly within it. 

#### Simple rightward RT as a proxy.
```{r}
names(covat)
covat$srt <- covat$R_V_50 + covat$R_IV_50 + covat$R_V_150 + covat$R_IV_150

t.test(data=covat, srt ~ Group)

ezCor(data.frame(covat$CES.L_50, covat$CES.L_150, covat$srt), label_size = 5)

```

The groups are not even remotely different on this measure. Further, 
it doesn't even marginally correlate with either CES value (both groups). 
Probably not useful as a "proxy" of CES.



#### Simple Winsorized leftward RT

```{r}
covat$lrt_50 <- (covat$L_V_50 + covat$L_IV_50)/2 
t.test(data=covat, lrt_50 ~ Group, alternative = "less")

covat$lrt_150 <- (covat$L_IV_150 + covat$L_IV_150)/2 
t.test(data=covat, lrt_150 ~ Group, alternative = "less")

ezCor(data.frame(covat$CES.L_50, covat$CES.L_150, covat$lrt_50, covat$lrt_150), label_size = 5)


cor.test(covat$CES.L_50, covat$lrt_50, method = "spearman")

cor.test(covat$CES.L_150, covat$lrt_150, method = "spearman")
```

The simple leftward 50 and 150ms SOA's correlate with one-another almost perfectly (.95), so they could 
probably just be collapsed to a single simple leftward RD.


```{r}
covat$lrt <- (covat$L_IV_150 + covat$L_IV_150 + covat$L_V_50 + covat$L_IV_50)/4
t.test(data=covat, lrt ~ Group, alternative = "less")
kruskal.test(covat$lrt ~ covat$Group)
cor.test((covat$CES.L_50 + covat$CES.L_150), covat$lrt, method="spearman")
```

Winsorizing the data:
```{r}
covat$lrt_50[is.na(covat$lrt_50)] <- mean(covat$lrt_50, na.rm=TRUE) + 
  sd(covat$lrt_50, na.rm=TRUE)*2

covat$lrt_150[is.na(covat$lrt_150)] <- mean(covat$lrt_150, na.rm=TRUE) + 
  sd(covat$lrt_150, na.rm=TRUE)*2

covat$lrt[is.na(covat$lrt)] <- mean(covat$lrt, na.rm=TRUE) + 
  sd(covat$lrt, na.rm=TRUE)*2
```
...
Conclusion: Simple leftward RT is not useful, either on it's own or 
winsorized, as it doesn't correlate reliably with CES, and, unlike CES,
isn't significantly different between the groups.
...
#names(AA)[3] <- "variable"
#CES_AA <- rbind(CES,AA)
COVAT_P <- dcast(CES, Subject + variable ~ .,
...
### Predict group based on COVAT and VWM
```{r}
CES_a <- dcast(CES, Subject + Group + variable ~ ., 
      subset=.(variable != "CES.R"), mean)[,c(1,2,4)]
names(VWM_A)[2] <- "VWM_NT"
names(VWM_SINGLE_A)[3] <- "VWM_SG"
VWM_SINGLE_A <- VWM_SINGLE_A[,c(1,3)]
VWM_A <- join(VWM_A, VWM_SINGLE_A)

names(CES_a) <- c("Subj_ID", "Group", "CES")
d <- join(CES_a, VWM_A, by="Subj_ID")

#Winsorize
d$CES[is.na(d$CES)] <- mean(d$CES, na.rm=TRUE) + 2*sd(d$CES, na.rm=TRUE)

```

```{r}
fit <- glm(Group~ CES,data=d,family=binomial())
...
fit2 <- glm(Group~ CES + VWM_NT,data=d,family=binomial())
fit3 <- glm(Group~ CES + VWM_SG,data=d,family=binomial())
fit4 <- glm(Group~ CES + VWM_NT + VWM_SG,data=d,family=binomial())
fit5 <- glm(Group~ CES + VWM_NT * VWM_SG,data=d,family=binomial())
fit6 <- glm(Group~ CES * VWM_NT * VWM_SG,data=d,family=binomial())
fit7 <- glm(Group~ log(CES) * log(VWM_NT) * log(VWM_SG),data=d,family=binomial())
fit8 <- glm(Group~ log(CES) + log(VWM_NT) +log(VWM_SG),data=d,family=binomial())
anova(fit,fit2, test="Chisq")
anova(fit,fit3, test="Chisq")
anova(fit3,fit4, test="Chisq")
anova(fit3,fit5, test="Chisq")
...
d2 <- d
shapiro.test(d2$CES)
d2$CES <- d2$CES + 1 -  min(d2$CES) #Shift CES to allow for log transform
shapiro.test(sqrt(d2$CES))
d2$VWM_SG <- factor(d$VWM_SG > median(d$VWM_SG))
d2$VWM_NT <- factor(d$VWM_NT > median(d$VWM_NT))
fit6b <- glm(Group~ CES * VWM_NT * VWM_SG,data=d2,family=binomial())
fit6c <- glm(Group~ CES + VWM_NT + VWM_SG,data=d2,family=binomial())
plot(fit6b)
anova(fit6b, test="Chisq")

plot(fit6c)
anova(fit6c, test="Chisq")
drop1(fit6c, test="Chisq")
library(xtable)
print.xtable(xtable(anova(fit6c, test="Chisq")), type="latex", 
             file="aod-raw.tex")
...
TESTS_P <- C.TESTS[C.TESTS$Group == "Patient",][,c(1,6,7,8)]
```
...
COR <- cbind(COR, TESTS_P, by="Subject", label_size = 5)[,c(1:4,6,7,8)]
ezCor(COR[,c(2:5,7)], label_size = 5)
```
...
```{r}
ezCor(COR[,c(2:4,7)], label_size = 5)
```
...
### Demographics and Clinical table (tbl_VWM)



```{r tbl_VWM}
require(gridExtra)
dem <- read.csv('../Patients.csv')
names(dem)[1] <- 'Subject'
dem <- dem[dem$Chapter == 'VWM',]
dem <- dem[-7] #date of injury
dem <- dem[-6] #date tested
dem <- dem[-2] #chapter
dem <- dem[1:4] #chapter


tbl <- COR[order(COR$Bisection),]
tbl$Bisection <- round(tbl$Bisection,1)
tbl$Stars <- round(tbl$Stars)
tbl$VWM_Single <- round(tbl$VWM_Single,2)
tbl$VWM_Multi <- round(tbl$VWM_Multi, 2)

tbl <- merge(dem, tbl, by="Subject")


names(tbl) <- c('Subject','Age','Sex','Handedness','CES',
                'VWM(1)','VWM(2/3)','Stars','Copying','Bisection')
tbl <- tbl[order(tbl$CES),]
rownames(tbl) <- tbl$Subject
tbl <- tbl[-1]

grid.table(tbl)

pdf("tbl_VWM.pdf", paper='USr', width=11)
grid.arrange(tableGrob(tbl), sub="tbl_VWM", clip=FALSE)
dev.off()

require(xtable)
newobject<-xtable(tbl)
print.xtable(newobject, type="latex", file="tbl_VWM-raw.tex")
```

### "Winsorization"
In order to provide values for those patients unable to re-orient leftward,
we could "winsorize" the data. 

```{r}
# Winsorize
COR$CES.L[is.na(COR$CES.L)] <- mean(COR$CES.L, na.rm=TRUE) + 
                                     2* sd(COR$CES.L, na.rm=TRUE)
```

Now, non-parametric correlations between CES, VWM1/2, and Bisection
```{r}
cor.test(COR$CES.L, COR$Bisection, method="spearman")
cor.test(COR$CES.L, COR$VWM_Single, method="spearman")
cor.test(COR$CES.L, COR$VWM_Multi, method="spearman")
cor.test(COR$VWM_Single, COR$Bisection, method="spearman")
cor.test(COR$VWM_Multi, COR$Bisection, method="spearman")
cor.test(COR$VWM_Single, COR$VWM_Multi, method="spearman")
```
...
### Plot the distributions to be used in fig_emrich2012. 
 
* Produce 3 figures
* Vertically stack
* Generate normal distributions, and plot density curves.

```{r}
norm <- data.frame(rnorm(100000, 4,sd= .8), 
                   c(rnorm(100000/2, 2,sd=.8), rnorm(100000/2, 9,sd=.8))
                   )
names(norm) <- c("T", "N")

require(ggplot2)
source("../Prisms/multiplot.r")

# 1. Single distribution at 'T'
m = ggplot(norm, aes(x=T)) +
  geom_density(adjust=5, size=1, fill=4, alpha=0.5) + coord_flip() +
  xlim(-1,15) + ylim(0,.6) + xlab("Probability Response") +
  ylab("") + theme_classic()


# 2. Flat distribution
n = ggplot() +
   geom_rect(aes(xmin=0, xmax=15, ymin=0, ymax=0.05), 
             alpha=0.5, color=1, fill=4, size=1) + coord_flip() +
   xlim(-1,15) + ylim(0,.6) + xlab("Probability Response") +
   ylab("") + theme_classic()
n

# 3. Two Distributions at 'N' and 'N'
o = ggplot(norm, aes(x=N)) +
  geom_density( size=1, fill=4, alpha=0.5, kernel = "biweight") +
  coord_flip() +
  xlim(-1,15) + ylim(0,.6) + xlab("") +
  ylab("") + theme_classic()

multiplot(m,n,o)

svg(filename="fig_Emrich2012_raw.svg", width=4, height=6)
multiplot(m,n,o)
dev.off()
